<?php

use GuzzleHttp\Client;
use GuzzleHttp\Exception\GuzzleException;
use Ufee\Amo\Oauthapi;

try {
    require_once 'vendor/autoload.php';

    file_put_contents('log-metric.log', date('Y-m-d H:i:s').' => '.json_encode($_POST)."\n", FILE_APPEND);

//    if ($_GET['env'] == 'test') {
//
//        $leadId = $_GET['lead_id'];
//    } else
//        $leadId = $_POST['status'][0]['id'] ?? $_POST['add'][0]['id'];

    $client_id = '154bbcc6-c232-44ba-a705-1bd375dba1d1';
    $client_secret = 'yovyYOLEhC5CuUbKoqU1ZLAoj7dpEG3zJzt1pt6sZ28C8ibk5OGKrs6jbeOMCFj2';
    $access_code = '';
    $amoCRM = Oauthapi::setInstance([
        'domain' => 'siriusfuture',
        'client_id' => $client_id,
        'client_secret' => $client_secret,
        'redirect_uri' => 'https://siriusfuture.ru/salid',
    ]);

    /*
     *  В работе 289135152
        Записан 289135153
        Проведен 289135154
        Оплачен 289135151
     */
    return (new Client())
        ->post('https://api-metrika.yandex.net/cdp/api/v1/counter/52207693/data/orders?merge_mode=APPEND', [
            'json' => [
                "orders" => [[
                    "id" => "32152",
                    "client_uniq_id"   => "sertw345fgdsg",
                    "client_type"      => "CONTACT",
                    "create_date_time" => "2023-05-17 10:17:00",
                    "update_date_time" => "2023-05-17 10:12:21",
                    "finish_date_time" => "2023-05-17 10:59:00",
                    "revenue"          => 1000,
                    "order_status"     => "289135152",
                    "cost"             => 500,
                    "products"         => [ "Товар А" => 173 ],
                    "attribute_values" => ["channel"  => "online"]
                ]]
            ]
        ]);

} catch (Exception $exception) {
} catch (GuzzleException $exception) {

    echo '<pre>';
    print_r($exception->getMessage());
    echo '</pre>';

    file_put_contents('log-metric-error.log',
        date('Y-m-d H:i:s') . ' => ' . $exception->getFile() . ' ' . $exception->getLine(
        ) . ' : ' . $exception->getMessage(),
        FILE_APPEND
    );
}